...
//       cond = ...; // ignored
//     }
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_2d_add)
{
...
//       cond = ...;
//     }
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_2d_no_identity_termination_cond)
{
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_2d_trip_count_max_int)
{
    const auto function = onnx_import::import_onnx_model(
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_2d_no_identity_termination_cond_static_shapes)
{
    const auto function = onnx_import::import_onnx_model(file_util::path_join(
        SERIALIZED_ZOO,
...
// cond = false
// input ("", cond) // Note this is analogous to a while loop
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_2d_no_identity_termination_cond_false)
{
    const auto function = onnx_import::import_onnx_model(file_util::path_join(
...
//        cond = ...;
//      }
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_2d_const_no_identity_termination_cond)
{
    const auto function = onnx_import::import_onnx_model(file_util::path_join(
...
//        cond = ...;
//      }
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_2d_both_cond_and_trip_count_as_inputs)
{
    const auto function = onnx_import::import_onnx_model(file_util::path_join(
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_add_initializer_from_parent_scope)
{
    const auto function = onnx_import::import_onnx_model(file_util::path_join(
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_add_node_from_parent_scope)
{
    const auto function = onnx_import::import_onnx_model(file_util::path_join(
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_add_value_access_to_body_scope_exception)
{
    try
    {
        const auto function = onnx_import::import_onnx_model(file_util::path_join(
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_add_value_the_same_node_from_parent_and_subgraph)
{
    const auto function = onnx_import::import_onnx_model(
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_add_input_from_parent_graph)
{
    const auto function = onnx_import::import_onnx_model(file_util::path_join(
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_the_proper_opset_in_subgraph)
{
    const auto function = onnx_import::import_onnx_model(
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_scalars)
{
    const auto function = onnx_import::import_onnx_model(
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_2d_add_const_cond)
{
    const auto function = onnx_import::import_onnx_model(
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_2d_trip_count_dynamic)
{
    const auto function = onnx_import::import_onnx_model(
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_concat_values)
{
    const auto function = onnx_import::import_onnx_model(
...
//       }
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_2d_trip_count_and_cond_skipped_shape_inference)
{
    const auto function = onnx_import::import_onnx_model(file_util::path_join(
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_infinite)
{
    const auto function = onnx_import::import_onnx_model(
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_no_variadic_inputs_and_outputs)
{
    const auto function = onnx_import::import_onnx_model(file_util::path_join(
...
NGRAPH_TEST(IE_GPU, onnx_controlflow_loop_power)
{
    const auto function = onnx_import::import_onnx_model(
