...
NGRAPH_TEST(INTERPRETER, proposal_v0_basic)
{
    ...
    const int image_z = 1;
    const std::vector<float> ratios = {0.5f};
    const std::vector<float> scales = {32.0f};
    ...
    auto class_probs_param = make_shared<op::Parameter>(element::f32, class_probs_shape);
    auto bbox_deltas_param = make_shared<op::Parameter>(element::f32, bbox_deltas_shape);
    auto image_shape_param = make_shared<op::Parameter>(element::f32, image_shape_shape);

    auto proposal = make_shared<op::v0::Proposal>(
    ...
    test_case.add_input<float>(class_probs_shape, c);
...
NGRAPH_TEST(INTERPRETER, proposal_v4_basic)
{
    const float iou_threshold = 0.7f;
    const int min_bbox_size = 16;
    const int feature_stride = 16;
    ...
    auto proposal = make_shared<op::v4::Proposal>(
        class_probs_param, bbox_deltas_param, image_shape_param, attrs);
    auto f = make_shared<Function>(
        proposal, ParameterVector{class_probs_param, bbox_deltas_param, image_shape_param});
    ...
    std::vector<float> o{std::begin(proposal_ref), std::end(proposal_ref)};
    std::vector<float> p{std::begin(probs_ref), std::end(probs_ref)};

    test_case.add_expected_output<float>(output_shape, o);
    test_case.add_expected_output<float>(probs_shape, p);
    test_case.run();
}
